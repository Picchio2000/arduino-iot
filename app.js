...
function handleBlockStatus(message) {
    ...
    if (isActive && !blockActive) {
        ...
        addLog('🚨 BLOCCO RILEVATO');
        
        // Send immediate notification via FCM
...
// Send block notification via FCM
async function sendBlockNotification(type, duration = null) {
    ...
    if ('Notification' in window && Notification.permission === 'granted') {
        const notification = new Notification(title, {
            ...
            icon: 'icon-192.png',
            badge: 'icon-192.png',
            vibrate: type === 'FIRST' ? [500, 200, 500, 200, 500] : [200, 100, 200],
            tag: 'block-alert',
    ...
    // For background notifications, FCM will be handled by service worker
    // We save the notification request to Firestore so server can send it
    try {
